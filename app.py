...
def apply_audio_modification2 (audio_path, filename):
    start_time = time.time()
    print("Debug 1")
    # Get the absolute path of the audio file
    audio_path = os.path.join(audio_path)
    ...
    def process_audio(audio_chunk):
        with torch.no_grad():
            noisy_audio_chunk = model(audio_chunk)

        noisy_audio_chunk = np.squeeze(noisy_audio_chunk.numpy())
        print("Debug 7")

        return noisy_audio_chunk

    # Process audio using a ThreadPoolExecutor for parallelism
    num_threads = 4  # You can adjust this based on your system's capabilities
    modified_audio_chunks = []
    print("Debug 8")

    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        audio_chunks = np.array_split(audio_tensor, num_threads)
        noisy_audio_chunks = executor.map(process_audio, audio_chunks)

        for chunk in noisy_audio_chunks:
            modified_audio_chunks.append(chunk)

    # Combine the modified audio chunks back into a single array
    modified_audio = np.concatenate(modified_audio_chunks)
    print("Debug 9")
    ...
    sf.write(modified_audio_path, modified_audio, sr)  # Assuming mono audio
...
def compare():
    ...
    if file and allowed_file(file.filename):
        ...
        modified_audio_path2 = apply_audio_modification2(file_path, file.filename)
